...
def add_me relations
	relations.keys.each do |person|
		parse "Josh would gain 0 happiness units by sitting next to #{person}", relations
		parse "#{person} would gain 0 happiness units by sitting next to Josh", relations 
	end	
end

def parse line, relations
	line.scan($expr).each do |a, mood, amount, b|
		relations[a][b] = amount.to_i * (mood <=> "i")
	end
end
...
DATA.each do |line|
	parse line, relations
end

add_me relations

puts relations.keys.permutation.map{ |arrangement|
	(arrangement << arrangement.first).each_cons(2).map{ |pair|
		relations[pair.first][pair.last] + relations[pair.last][pair.first]
	}.inject(:+)
}.max
