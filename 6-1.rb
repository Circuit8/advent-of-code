...
class LightGrid
	...
	def show
		@grid.each do |row|
			row.each do |cell|
				if cell
					print "#{cell}".colorize(:yellow)
				else
					print "#{cell}".colorize(:black)
				end
				print "\t"
			end
			puts "\n\n"
	...
	def read_instructions instructions
		instructions.gsub!("turn ", "")
		instructions_array = instructions.split(" ")

		switch_setting = instructions_array[0]
		start_coords 	= parse_coords instructions_array[1]
		end_coords 		= parse_coords instructions_array[3]

		switch_lights switch_setting, start_coords, end_coords
	end
	...
	private
		...
		def switch_lights switch_setting, start_coords, end_coords
			@grid[start_coords[1]..end_coords[1]].each do |row|
				index = start_coords[0]
				row[start_coords[0]..end_coords[0]].each{ |cell| 
					row[index] = switch_light cell, switch_setting
					index += 1
				}
			end
		end

		def switch_light old_light, instruction
			case instruction
			when "on"
				return old_light + 1		
			when "off"
...
lights = LightGrid.new(1000)

DATA.each do |instructions|
	lights.read_instructions instructions
end

puts lights.num_lights_on
